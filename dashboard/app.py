...
def load_and_process(path):
    ...
    X_scaled = scaler.fit_transform(
        rfm_log[['Recence', 'Frequence', 'Montant']]
    )
    km = KMeans(n_clusters=4, random_state=42, n_init=10)
    rfm['Cluster'] = km.fit_predict(X_scaled)
    moyennes = rfm.groupby('Cluster')['Recence'].mean()
    montants = rfm.groupby('Cluster')['Montant'].mean()
    segment_map = {}
    segment_map[moyennes.idxmin()] = 'Champions'
    segment_map[moyennes.idxmax()] = 'Perdus'
    remaining = [c for c in range(4) if c not in segment_map]
    segment_map[montants[remaining].idxmax()] = 'A risque'
    segment_map[montants[remaining].idxmin()] = 'Nouveaux prometteurs'
    rfm['Segment'] = rfm['Cluster'].map(segment_map)
    return rfm

rfm = load_and_process('data/online_retail_clean.csv')
...
with col2:
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(segs, fontsize=9, rotation=15, ha='right')
